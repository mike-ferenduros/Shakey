...
public:
  ...
  void setCleanup (int mode)
    { dSpaceSetCleanup (oid(), mode); }
  int getCleanup()
    { return dSpaceGetCleanup (oid()); }
  ...
  void remove (dGeomID x)
    { dSpaceRemove (oid(), x); }
  int query (dGeomID x)
    { return dSpaceQuery (oid(),x); }

  int getNumGeoms()
    { return dSpaceGetNumGeoms (oid()); }
  dGeomID getGeom (int i)
    { return dSpaceGetGeom (oid(),i); }

  void collide (void *data, dNearCallback *callback)
    { dSpaceCollide (oid(),data,callback); }
};
...
public:
  ...
  dSimpleSpace (dSpace &space)
    { _id = (dGeomID) dSimpleSpaceCreate (space.oid()); }
...
public:
  ...
  dHashSpace (dSpace &space)
    { _id = (dGeomID) dHashSpaceCreate (space.oid()); }
  dHashSpace (dSpaceID space)
    { _id = (dGeomID) dHashSpaceCreate (space); }

  void setLevels (int minlevel, int maxlevel)
    { dHashSpaceSetLevels (oid(),minlevel,maxlevel); }
};
...
public:
  ...
  dQuadTreeSpace (dSpace &space, const dVector3 center, const dVector3 extents, int depth)
    { _id = (dGeomID) dQuadTreeSpaceCreate (space.oid(),center,extents,depth); }
...
public:
  ...
  dSphere (dSpace &space, dReal radius)
    { _id = dCreateSphere (space.oid(), radius); }
...
public:
  ...
  dPlane (dSpace &space, dReal a, dReal b, dReal c, dReal d)
    { _id = dCreatePlane (space.oid(),a,b,c,d); }
...
public:
  ...
  dCapsule (dSpace &space, dReal radius, dReal length)
    { _id = dCreateCapsule (space.oid(),radius,length); }
...
public:
  ...
  dCylinder (dSpace &space, dReal radius, dReal length)
    { _id = dCreateCylinder (space.oid(),radius,length); }
...
public:
  ...
  dRay (dSpace &space, dReal length)
    { _id = dCreateRay (space.oid(),length); }
...
public:
  ...
  dGeomTransform (dSpace &space)
    { _id = dCreateGeomTransform (space.oid()); }
